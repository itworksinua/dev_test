...
import { IAsset, IPreviewImage } from 'chowchow/src/lib/interfaces'
...
import VideoPlayer from "chowchow/src/components/VideoPlayer"
...
class ShowVideos extends React.Component<EmptyListProps, any> {
  ...
  public render() {
    ...
    const children = videos.map((video, i) => buildAssetRow(productId, video))
...
function buildAssetRow(productId: number, asset: IAsset) {
  ...
  return (
    <View style={styles.row} key={`${asset.id}`}>
      <TouchableOpacity style={styles.touchable} onPress={() => showVideo(videoItem) }>
